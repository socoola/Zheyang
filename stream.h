...
struct stream_destination {
	...
	struct stream *stream;/* source stream, output of a slot */
	int waiting;/* waiting for data? 1---yes, 0---no */
	frame_deliver_func process_frame;
	void *d;/*arg of process_frame */
};
...
struct stream {
	...
	char name[256];
	int format;
	struct stream_destination *dest_list;/* output sink list */
    void (*get_property)(struct stream *s, int argname, void *arg, int *arglen);
	void (*get_framerate)( struct stream *s, int *fincr, int *fbase );
	void (*set_running)( struct stream *s, int running );
	void *private;/* slot entry */
};
