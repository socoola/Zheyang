...
#define RTP_TRANS_MC    3 // multicast mode
...
struct rtp_endpoint {
    ...
    unsigned int recv_start_ts;
	int seqnum;
	int packet_count;
	int octet_count;
    unsigned char send_drop_rate;
	struct event *rtcp_send_event;
	int force_rtcp;
	struct timeval last_rtcp_recv;
	int trans_type;
    bool bLostKeyFrame;
    bool bTriggerWirelessCheck;
	union {
		struct {  
            struct in_addr dest_ip;         // network endian
            unsigned short dest_rtp_port;   // network endian
            unsigned short dest_rtcp_port;  // network endian
			char sdp_addr[48];
			int sdp_port;
			int rtp_fd;
...
typedef int (*get_sdp_func)( struct session *s, char *dest, int *len,
					char *path );
typedef int (*set_sdp_func)( struct session *s, struct pmsg *pmsg,
					char *path );
...
typedef void (*record_data_fun)( struct session *s, struct rtp_endpoint *ep, struct frame *pFrame);
#define	MAX_TRACKS	2
...
struct session {
    ...
    set_sdp_func set_sdp;
	setup_func setup;
	play_func play;
    record_data_fun record_data;
	pause_func pause;
