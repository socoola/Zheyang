			...
			frame_deliver_func process_frame, void *d )
{
    ...
    spook_log( SL_ERR, "connect to stream %s", s->name);
...
/* connect to a stream to receive the output of stream */
...
/* set the waiting(runing) state of current sink, 
 * recursively set up-layer's state if the other sinks 
 * of the same stream is in the same state
 */
