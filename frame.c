...
struct event *g_pEvent = NULL;
unsigned int g_ulEventTime = 0;

void init_frame_heap( int size, int count )
{
	struct frame_slot *f = NULL, *prev = NULL;

    printf("\r\nbegin init frame heap!\r\n");
...
void deinit_frame_heap()
{
    struct frame_slot *fs, *temp;
    pthread_mutex_lock( &frame_heap_mutex );
    //@TODO:free frame slot
    pthread_mutex_unlock( &frame_heap_mutex );
    pthread_mutex_destroy( &frame_heap_mutex );
}
...
struct frame *new_frame(void)
{
	...
	if( frame_heap_take->f )
	{
		...
		f->d = NULL;//(unsigned char *)f + sizeof( struct frame );
...
void exchanger_read( struct event_info *ei, void *d )
{
	...
	for(;;)
	{
        ...
        ex->nReads++;
        ex->tRead[(ex->nRead++) & 0x3F];
		pthread_mutex_unlock( &ex->mutex );
        //spook_log( SL_INFO, "ex:%08x, exchange_read:%08x", ex, f);
        tStart = get_tickcount();
		ex->f( f, ex->d );
        tNow = get_tickcount();
        if((tNow - tStart) > g_ulChkTv)
        {
            //printf("\r\nExchange Event Error:%d!!!!\r\n", (tNow - tStart));            
        }
	}
}

/* create an exchanger, waiting for data and deliever data to d by call func. 
 * user can handle data in func, usually call d's process_frame in func
 * to trigger d to process data as an input-source.
 * user call deliver_frame to write data to the exchanger
 * slots: max queue length
 * func: function will be called when receive a frame
 * d: destination of exchanger, user tranfer data to it by exchanger
 */
					...
					frame_deliver_func func, void *d )
{
    ...
    // produce frame slots(without frame) and link them
	while( slots-- > 0 )
	{
		f = (struct frame_slot *)
				malloc( sizeof( struct frame_slot ) );
    ...
    // link the frame slots(double linked, finally step)
	ex->slave_cur->prev = f;
	f->next = ex->slave_cur;
	ex->master_read = ex->master_write = ex->slave_cur;

    // create a pipe for exchange
    //@TODO: master is the input slot, slave is the output slot
	pipe( fds );
	ex->master_fd = fds[0];
	ex->slave_fd = fds[1];
    ...
    ex->nGets = 0;
    ex->nReads = 0;
    ex->nRead = ex->nWrite = 0;
    memset(ex->tWrite, 0, sizeof(ex->tWrite));
    memset(ex->tRead, 0, sizeof(ex->tRead));
...
void del_exchanger(struct frame_exchanger *ex)
{
    ...
    pthread_cond_destroy(&ex->slave_wait);
    close(ex->master_fd);
    close(ex->slave_fd);
...
// provide a new frame for get_next_frame
int exchange_frame( struct frame_exchanger *ex, struct frame *frame )
{
    ...
    ex->nExchanges++;
	pthread_mutex_unlock( &ex->mutex );
    //spook_log( SL_INFO, "ex:%08x, exchange_frame:%08x", ex, frame);

	return 0;
}

// get a frame that exchanged(triggered by exchange_frame)
struct frame *get_next_frame( struct frame_exchanger *ex, int wait )
{
	struct frame *f = NULL;

	pthread_mutex_lock( &ex->mutex );
	if( ex->slave_cur->pending ) 
        f = ex->slave_cur->f;
	if( ! f && wait )
	{
    ...
    {
        /*
        struct frame_slot *pfs = ex->slave_cur;
        
        do
        {
            spook_log( SL_INFO, "ex:%08x, has frame:%08x, state:%d", ex, pfs->f, pfs->pending);
            pfs = pfs->next;
        }while(pfs != ex->slave_cur);
        spook_log( SL_WARN, "Event type:%d, flag:%d", ex->master_event->type, ex->master_event->flags);
        
        spook_log( SL_INFO, 
        "ex:%08x, read:%d, deliver:%d, exchange:%d, get:%d", 
        ex,
        ex->nReads, ex->nDelivers, ex->nExchanges, ex->nGets);
        */
        if(!g_pEvent)
        {
            g_ulEventTime = get_tickcount();
            g_pEvent = ex->master_event;
        }
    }
	pthread_mutex_unlock( &ex->mutex );    
        //spook_log( SL_INFO, "ex:%08x, get_next_frame:%08x", ex, f);
	return f;
}

// trigger exchanger_read
void deliver_frame( struct frame_exchanger *ex, struct frame *f )
{
	unsigned char c = 0;

    //@TODO: why do not use mutex and signal slave_wait?
    // deliver a frame to master_read, then notify by using pipe
    //spook_log( SL_INFO, "ex:%08x, deliver frame:%08x", ex, f);
    pthread_mutex_lock( &ex->mutex );
    if(ex->slave_cur->f)
        spook_log( SL_INFO, "ex:%08x, error deliver frame:%08x", ex, f);
	ex->slave_cur->f = f;
	ex->slave_cur->pending = 0;
	if( write( ex->slave_fd, &c, 1 ) <= 0 ) 
	{
        spook_log( SL_INFO, "unknown problem error!!!");
        pthread_mutex_unlock( &ex->mutex );
        exit( 0 );
	}
