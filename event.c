...
extern struct event *g_pEvent;
extern unsigned int g_ulEventTime;
...
unsigned int g_ulChkTv = 2;
void event_loop( int single )
{
	struct timeval t, *st, nowait;
	struct event *e;
	...
	do {
		...
		for( e = always_event_list; e; e = e->next )
		{
            ...
            tNow = get_tickcount();
                if((tNow - tStart) > g_ulChkTv)
                {
                    //printf("\r\nAlways Event Error:%d!!!!\r\n", (tNow - tStart));
                }
		}
		if( ret > 0 ) for( e = fd_event_list; e; e = e->next )
		{/*
            if(g_pEvent && (e == g_pEvent))
            {                
                spook_log(SL_INFO, "%08x set, %08x now, Event Info:flags->%d shot-->%d", 
                    g_ulEventTime,
                    get_tickcount(),
                    e->flags,
                    FD_ISSET( e->ev.fd.fd, e->ev.fd.write ? &wfds : &rfds ));
                g_pEvent = NULL;
            }
           */     
			if( ! ( e->flags & EVENT_F_RUNNING ) ) continue;
					...
					e->ev.fd.write ? &wfds : &rfds ) )
			{
                ...
                if((tNow - tStart) > g_ulChkTv)
                {
                    extern void exchanger_read( struct event_info *ei, void *d );
                    //printf("\r\nFD Event Error:%d!!!!\r\n", (tNow - tStart));
                    if(e->func == exchanger_read)
                        ;//printf("\r\nExchange_read\r\n");
                }
			}
		}
        ...
        if((tNow - tStart) >= 50)
        {
            printf("\r\nEvent process timeout:%d", tNow - tStart);
        }
