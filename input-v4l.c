...
/*
	...
	{
		spook_log( SL_ERR, "v4l: aborting on error in %d VIDIOCMCAPTURE: %s",
				conf->fd, strerror( errno ) );
		exit( 1 );
	}
	printf("Begin to capture frame %d at %d\r\n", cur_buf, conf->fd );
...
*/
	for(;;)
	{
		...
		//cur_buf = cur_buf ^ 1;
		printf("Begin to sync frame %d at %d\r\n", cur_buf, conf->fd);
		if( ioctl( conf->fd, VIDIOCSYNC, &cur_buf ) < 0 )
		{
			spook_log( SL_ERR,
				"v4l: aborting on error in %d VIDIOCSYNC: %s",
				conf->fd, strerror( errno ) );
			exit( 1 );
		}
...
static int v4l_setup( struct v4l_input *conf )
{
	...
	int i, pwc, real_width, real_height, cur_buf;
#ifdef HAVE_PWC_IOCTL_H
...
/*
	...
	{
		spook_log( SL_ERR, "v4l: error selecting input port:%s", strerror( errno ));
		return -1;
	}
*/
	...
	if( ! ( conf->mmap_buf = (unsigned char *)mmap( NULL, conf->vm.frames, PROT_READ, MAP_SHARED, conf->fd, 0 ) ) )
	{
	...
	{
		cur_buf = i;
		ioctl( conf->fd, VIDIOCSYNC, &cur_buf );
	}
