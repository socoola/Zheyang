						...
						char *path, int len )
{
	...
	{
        nStrLen = strlen(loc->path);
		if( ! strncmp( path, loc->path, len > nStrLen ? len : nStrLen )
				&& ( loc->path[len] == '/'
					|| loc->path[len] == 0 ) )
			return loc;
	}
...
int interleave_recv( struct conn *c, int chan, unsigned char *d, int len )
{
    ...
    {
        struct rtp_endpoint *ep = rc->ichan[chan].ep;
        struct frame *f;
        if(ep->ex)
        {
            ...
            if(f)
            {
                f->length = (len - 12) > ep->max_data_size ? ep->max_data_size:(len - 12);
        		f->format = 0;
        		f->width = 0;
        		f->height = 0;
        		f->key = (d[1] & 0x80)?1:0;
                f->d = (unsigned char *)malloc(f->length);
                if(f->d)
                {
                    memcpy(f->d, d, f->length);
...
static int handle_DESCRIBE( struct req *req )
{
    ...
    spook_log( SL_INFO, "find location:%s", loc->node.path);
	...
	} else rtsp_send_error( req, 404, "SDP Not Found" );
...
static int handle_ANNOUNCE( struct req *req )
{
    char path[256];
	struct rtsp_location *loc;
	struct session *sess;
    ...
    if( sess->set_sdp( sess, req->req, req->req->sl.req.uri ) > 0 )
    {
        rtsp_create_reply( req, 200, "OK" );
        tcp_send_pmsg( req->conn, req->resp, -1 );    	
    }
...
static int rtsp_udp_setup( struct session *s, int track,
				struct req *req, char *t, int bMulticast )
{
    ...
    {
        sprintf( trans, 
            "Transport: RTP/AVP;multicast;destination=%s;port=%d-%d;ttl=1",
            s->ep[track]->trans.udp.sdp_addr, 
            ntohs(s->ep[track]->trans.udp.dest_rtp_port), 
            ntohs(s->ep[track]->trans.udp.dest_rtcp_port) );
...
static int handle_SETUP( struct req *req )
{
    ...
    //@TODO: ep will established in s->setup
	if( s->ep[track] )
	{
		// XXX better error reply
		rtsp_send_error( req, 461, "Unsupported Transport1" );
		return 0;
	}

    if((get_param(t, "mode", path, sizeof(path)) > 0) 
        && ((strncasecmp(path, "receive", strlen("receive")) == 0)
            || (strncasecmp(path, "record", strlen("record")) == 0)))
    {
        s->bRecord = 1;
    }
	...
	rtsp_create_reply( req, 200, "OK" );
    if(s->ep[track]->trans_type == RTP_TRANS_MC)
    {
        ret = rtsp_udp_setup( s, track, req, t, 1 );
    }
    else
    {
    	if( ! strncasecmp( t, "RTP/AVP/TCP", 11 ) )
    		...
    		ret = rtsp_udp_setup( s, track, req, t, 0 );
    	else ret = -1;
    }
...
int rtsp_handle_msg( struct req *req )
{
    ...
    else if( ! strcasecmp( req->req->sl.req.method, "ANNOUNCE" ) )
    {
        ret = handle_ANNOUNCE( req );
    }
